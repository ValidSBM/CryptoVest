...
if (userNameSpan || balanceSpan || depositBtn || withdrawBtn) {
  ...
  if (depositBtn) {
    depositBtn.addEventListener('click', () => {
      let amount = parseFloat(prompt("Enter amount to deposit (USD):"));
      if (!isNaN(amount) && amount > 0) {
        const updatedUser = JSON.parse(localStorage.getItem('currentUser'));
        updatedUser.balance = (updatedUser.balance || 0) + amount;
        localStorage.setItem('currentUser', JSON.stringify(updatedUser));
        if (balanceSpan) {
          balanceSpan.textContent = `$${updatedUser.balance.toFixed(2)}`;
        }
  ...
  if (withdrawBtn) {
    withdrawBtn.addEventListener('click', () => {
      ...
      let amount = parseFloat(prompt("Enter amount to withdraw (USD):"));
      if (!isNaN(amount) && amount > 0 && amount <= (currentUserData.balance || 0)) {
        currentUserData.balance = (currentUserData.balance || 0) - amount;
        localStorage.setItem('currentUser', JSON.stringify(currentUserData));
        if (balanceSpan) {
          balanceSpan.textContent = `$${currentUserData.balance.toFixed(2)}`;
        }
